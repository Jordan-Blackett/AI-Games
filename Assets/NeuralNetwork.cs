...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
    ...
    {
        // Get a crossover cutting point
        int cutPoint = UnityEngine.Random.Range(0, parentA.Length - 1);
        ...
        float[] parentAOffSpring = parentA;
        float[] parentBOffSpring = parentB;
        for (var i = cutPoint; i < parentA.Length; i++)
        {
            //var biasFromParentA = parentA.neurons[i]['bias'];
            parentAOffSpring[i] = parentB[i];
            parentBOffSpring[i] = parentA[i];
        }

        return UnityEngine.Random.Range(0, 1) == 1 ? parentAOffSpring : parentBOffSpring;
    }

    public void ReadWeights(float[] offSpringWeights)
    {
        int tempIndex = 0;
        for (int i = 0; i < weights.Length; i++)
        {
            for (int j = 0; j < weights[i].Length; j++)
            {
                for (int k = 0; k < weights[i][j].Length; k++)
                {
                    weights[i][j][k] = offSpringWeights[tempIndex];
                    tempIndex++;
                }
            }
        }
